...
public class PassiveEffectHandler {
    ...
    public void applyOnHitPassive(WeaponVariant variant, Player attacker, LivingEntity target, double baseDamage) {
        ...
        switch (type) {
            ...
            case "STEAM_CLOUD" -> { int dur = toInt(cfg.getOrDefault("duration", 4)) * 20; target.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, dur, toInt(cfg.getOrDefault("blindness_level",2))-1)); target.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, dur, toInt(cfg.getOrDefault("slowness_level",2))-1)); }
            ...
            case "FROST_SLOW" -> target.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 60, toInt(cfg.getOrDefault("slow_level",2))-1));
